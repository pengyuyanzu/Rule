#!/bin/bash
# =============================================================================
# Script Name: SSH-Hardener-MobileAdaptation.sh
# Description: The "Mobile Adaptation" - Port Listen Check Only (No Loopback).
# Version:     72.0 (Final Golden Master - Single Session Optimized)
# Modification: verify_ssh_handshake now only checks if port is listening.
...
# [Fix] 降级验证：只检查端口是否监听，不进行协议握手
# 专门为单窗口/移动端/无ssh客户端环境优化
verify_ssh_handshake() {
    local p="$1"
    if is_listening "$p"; then
        info "检测到端口 $p 已处于监听状态。"
        return 0
    else
        warn "端口 $p 未处于监听状态。"
        return 1
...
echo -e "${GREEN}=== SSH 配置安全向导 (v72.0 Mobile Adaptation) ===${NC}"
echo -e "目标用户: ${CYAN}$TARGET_USER${NC}"
echo -e "目标配置: ${CYAN}$MAIN_CONF${NC}"
log_sys "Starting SSH hardening v72.0 for user: $TARGET_USER on config: $MAIN_CONF"
...
generate_secure_config() {
  echo "# Generated by Secure-Init-MobileAdaptation"
  echo "Port $SSH_PORT"
